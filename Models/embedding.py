...
class Embedding(nn.Module):
    def __init__(self, d_word: int, d_char: int, dropout: float, dropout_char: float, init_name: str = "kaiming", act_name: str = "relu"):
        ...
        # Character convolution should be applied *within each token* across
        # the character dimension, not across neighbouring tokens. We therefore
        # reshape [B, L, char_len, d_char] -> [B*L, d_char, char_len] and use a
        # 1-D depthwise separable convolution over char_len.
